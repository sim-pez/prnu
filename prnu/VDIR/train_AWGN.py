...
class Train(object):
    ...
    def _parse_function(self, example_proto):
        keys_to_features = {'label': tf.io.FixedLenFeature([], tf.string)}

        parsed_features = tf.io.parse_single_example(example_proto, keys_to_features)

        img = parsed_features['label']
        img = tf.divide(tf.cast(tf.io.decode_raw(img, tf.uint8), tf.float32), 255.)
        img = tf.reshape(img, [self.HEIGHT, self.WIDTH, self.CHANNEL])

        return img

    def load_tfrecord(self):
        dataset = tf.compat.v1.data.TFRecordDataset(self.tfrecord_path)
